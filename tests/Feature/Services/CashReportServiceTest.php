 ...
 * ***************************************************************************
 */
it('correctly updates report when expense date is updated to future date', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData();
    $expense = Expense::where('expense_date', '2025-03-06')->where('is_cash', true)->first();
    ...
    $expense->expense_date = Carbon::createFromFormat('Y-m-d', $date)->addDay()->format('Y-m-d');
    $expense->save();

    $beforeReports = CashReport::whereDate('date', '<', '2025-03-06')->orderBy('date')->get();
    $reports = CashReport::whereDate('date', '2025-03-06')->orderBy('date')->get();
    $afterReportOneDay = CashReport::whereDate('date', '2025-03-07')->orderBy('date')->get();
    $afterReports = CashReport::whereDate('date', '>', '2025-03-07')->orderBy('date')->get();

    expect(count($beforeReports))->toBe(4)
        ->and(count($reports))->toBe(1)
        ->and(count($afterReportOneDay))->toBe(1)
        ->and(count($afterReports))->toBe(1)
        ->and(CashReport::count())->toBe(7);
    ...
    foreach ($reports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ->and($report->cash_income)->toBe($data[$item]['cash_income'])
            ->and($report->cashless_income)->toBe($data[$item]['cashless_income'])
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'] - ($isCash ? $amount : 0))
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'] - ($isCash ? 0 : $amount))
            ->and($report->cash_salary)->toBe($data[$item]['cash_salary'])
            ->and($report->cashless_salary)->toBe($data[$item]['cashless_salary']);
        $item++;
    }

    foreach ($afterReportOneDay as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'] + ($isCash ? $amount : 0))
            ->and($report->cash_income)->toBe($data[$item]['cash_income'])
            ->and($report->cashless_income)->toBe($data[$item]['cashless_income'])
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'] + ($isCash ? $amount : 0))
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'] + ($isCash ? 0 : $amount))
...
it('correctly updates report when expense date is updated to past date', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData();
    $expense = Expense::where('expense_date', '2025-03-06')->where('is_cash', true)->first();
    ...
    $amount = $expense->expense_amount;
    $isCash = $expense->is_cash;
    $expense->expense_date = Carbon::createFromFormat('Y-m-d', $date)->subDays(1)->format('Y-m-d');
    $expense->save();

    $beforeReports = CashReport::whereDate('date', '<', '2025-03-05')->orderBy('date')->get();
    $beforeReportsOneDay = CashReport::whereDate('date', '2025-03-05')->orderBy('date')->get();
    $reports = CashReport::whereDate('date', '2025-03-06')->orderBy('date')->get();
    $afterReports = CashReport::whereDate('date', '>', '2025-03-06')->orderBy('date')->get();

    expect(count($beforeReports))->toBe(3)
        ->and(count($beforeReportsOneDay))->toBe(1)
        ->and(count($reports))->toBe(1)
        ->and(count($afterReports))->toBe(2)
        ->and(CashReport::count())->toBe(7);
    ...
    foreach ($beforeReportsOneDay as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ->and($report->cash_income)->toBe($data[$item]['cash_income'])
            ->and($report->cashless_income)->toBe($data[$item]['cashless_income'])
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'] + ($isCash ? $amount : 0))
 ...
 * ***************************************************************************
 */
it('correctly updates report when salary date is updated to future date', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData(isExpense: false);
    $salary = Salary::where('salary_date', '2025-03-06')->where('is_cash', true)->first();
    ...
    $salary->salary_date = Carbon::createFromFormat('Y-m-d', $date)->addDay()->format('Y-m-d');
    $salary->save();
    ...
    foreach ($reports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ...
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'])
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'])
            ->and($report->cash_salary)->toBe($data[$item]['cash_salary'] - ($isCash ? $amount : 0))
            ->and($report->cashless_salary)->toBe($data[$item]['cashless_salary'] - ($isCash ? 0 : $amount));
        $item++;
    }
...
it('correctly updates report when salary date is updated to past date', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData(isExpense: false);
    $salary = Salary::where('salary_date', '2025-03-06')->where('is_cash', true)->first();
    ...
    $amount = $salary->salary_amount;
    $isCash = $salary->is_cash;
    $salary->salary_date = Carbon::createFromFormat('Y-m-d', $date)->subDays(1)->format('Y-m-d');
    $salary->save();
...
it('correctly updates report when expense is updated from cash to cashless', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData();
    $expense = Expense::where('expense_date', '2025-03-06')->where('is_cash', true)->first();
    $cashAmount = $expense->expense_amount;
    $expense->is_cash = false;
    $expense->save();
    ...
    foreach ($reports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ...
            ->and($report->cashless_income)->toBe($data[$item]['cashless_income'])
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'] - $cashAmount)
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'] + $cashAmount)
            ->and($report->cash_salary)->toBe($data[$item]['cash_salary'])
            ->and($report->cashless_salary)->toBe($data[$item]['cashless_salary']);
        $item++;
    }
...
it('correctly updates report when expense is updated from cashlsess to cash', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData();
    $expense = Expense::where('expense_date', '2025-03-06')->where('is_cash', false)->first();
    $cashAmount = $expense->expense_amount;
    $expense->is_cash = true;
    $expense->save();
...
it('correctly updates report when expense is updated from cash to cashless with changed amount', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData();
    $expense = Expense::where('expense_date', '2025-03-06')->where('is_cash', true)->first();
    ...
    foreach ($reports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ...
            ->and($report->cashless_income)->toBe($data[$item]['cashless_income'])
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'] - $cashAmount)
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'] + $cashAmount + 500)
            ->and($report->cash_salary)->toBe($data[$item]['cash_salary'])
            ->and($report->cashless_salary)->toBe($data[$item]['cashless_salary']);
        $item++;
    }
...
it('correctly updates report when expense is updated from cashlsess to cash with changed amount', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData();
    $expense = Expense::where('expense_date', '2025-03-06')->where('is_cash', false)->first();
    $cashAmount = $expense->expense_amount;
    $expense->expense_amount = $cashAmount - 100;
    $expense->is_cash = true;
    $expense->save();
    ...
    foreach ($reports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ->and($report->cash_income)->toBe($data[$item]['cash_income'])
            ->and($report->cashless_income)->toBe($data[$item]['cashless_income'])
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'] + $cashAmount - 100)
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'] - $cashAmount)
            ->and($report->cash_salary)->toBe($data[$item]['cash_salary'])
            ->and($report->cashless_salary)->toBe($data[$item]['cashless_salary']);
        $item++;
    ...
    foreach ($afterReports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'] - $cashAmount + 100)
            ->and($report->cash_income)->toBe($data[$item]['cash_income'])
            ->and($report->cashless_income)->toBe($data[$item]['cashless_income'])
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'])
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'])
...
it('correctly updates report when salary is updated from cash to cashless', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData(isExpense: false);
    $salary = Salary::where('salary_date', '2025-03-06')->where('is_cash', true)->first();
    $cashAmount = $salary->salary_amount;
    $salary->is_cash = false;
    $salary->save();
    ...
    foreach ($reports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ...
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'])
            ->and($report->cash_salary)->toBe($data[$item]['cash_salary'] - $cashAmount)
            ->and($report->cashless_salary)->toBe($data[$item]['cashless_salary'] + $cashAmount);
        $item++;
    }
    foreach ($afterReports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'] + $cashAmount)
...
it('correctly updates report when salary is updated from cashlsess to cash', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData(isExpense: false);
    $salary = Salary::where('salary_date', '2025-03-06')->where('is_cash', false)->first();
    $cashAmount = $salary->salary_amount;
    $salary->is_cash = true;
    $salary->save();
...
it('correctly updates report when salary is updated from cash to cashless with changed amount', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData(isExpense: false);
    $salary = Salary::where('salary_date', '2025-03-06')->where('is_cash', true)->first();
    ...
    foreach ($reports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ...
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'])
            ->and($report->cash_salary)->toBe($data[$item]['cash_salary'] - $cashAmount)
            ->and($report->cashless_salary)->toBe($data[$item]['cashless_salary'] + $cashAmount + 500);
        $item++;
    }
    foreach ($afterReports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'] + $cashAmount)
...
it('correctly updates report when salary is updated from cashlsess to cash with changed amount', function () {
    $data = prepareCashReportData();
    preparePaymentData();
    prepareExpenseOrSalaryData(isExpense: false);
    $salary = Salary::where('salary_date', '2025-03-06')->where('is_cash', false)->first();
    $cashAmount = $salary->salary_amount;
    $salary->salary_amount = $cashAmount - 100;
    $salary->is_cash = true;
    $salary->save();
    ...
    foreach ($reports as $report) {
        expect($report->morning_cash_balance)->toBe($data[$item]['morning_cash_balance'])
            ...
            ->and($report->cash_expense)->toBe($data[$item]['cash_expense'])
            ->and($report->cashless_expense)->toBe($data[$item]['cashless_expense'])
            ->and($report->cash_salary)->toBe($data[$item]['cash_salary'] + $cashAmount - 100)
            ->and($report->cashless_salary)->toBe($data[$item]['cashless_salary'] - $cashAmount);
        $item++;
    }
    foreach ($afterReports as $report) {
